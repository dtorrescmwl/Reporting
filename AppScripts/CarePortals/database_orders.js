...
// Helper function to determine pharmacy based on shipping state
function getPharmacy(provinceCode) {
  return (provinceCode === 'SC' || provinceCode === 'IN') ? 'Other' : 'SevenCells';
}
...
function createOrder(spreadsheet, orderData, customerId, addressId, productId) {
  ...
  const pharmacy = getPharmacy(orderData.shippingAddress.provinceCode);
...
// Function to convert existing addresses to MD5 hashes
function convertExistingAddressesToMD5() {
  console.log("Starting conversion of existing addresses to MD5 hashes...");
  
  const spreadsheet = SpreadsheetApp.openById("1PaCOgNJmKSb2VsJjCMjWpCG-gwrO7uVkuTAFUdrV-_o");
  const sheet = getOrCreateSheet(spreadsheet, "addresses");
  
  if (sheet.getLastRow() <= 1) {
    console.log("No address data found to convert");
    return;
  }
  ...
  // Find column indices (assuming current structure)
  ...
  console.log("Found address columns, processing rows...");
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const oldAddressId = row[0]; // Current address_id
    
    console.log(`Row ${i}: "${oldAddressId}" → "${newAddressId}"`);
    
    // Update the address_id in column A
    sheet.getRange(i + 1, 1).setValue(newAddressId);
  }
  
  console.log("Conversion completed!");
  console.log("IMPORTANT: You need to manually update the order.created sheet to reference the new address IDs");
  console.log("Check the console output above to see the old → new ID mappings");
}
